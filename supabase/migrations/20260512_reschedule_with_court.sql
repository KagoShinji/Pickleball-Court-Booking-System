create or replace function public.reschedule_booking_atomic(
  ...
  p_new_court_id uuid default null
)
...
begin
  ...
  -- Determine target court (new court if provided, else keep existing)
  v_target_court_id := coalesce(p_new_court_id, v_existing_booking.court_id);

  select coalesce(type, '')
  into v_court_type
  from public.courts
  where id = v_target_court_id;
  ...
  -- Admin blocked conflicts logic for target court
  ...
  where bts.blocked_date = p_new_date
    and (
      (not v_is_exclusive and bts.court_id = v_target_court_id)
      or v_is_exclusive
    )
  ...
  set
    ...
    rescheduled_from = jsonb_build_object(
      ...
      'original_court_id', v_existing_booking.court_id,
      'reason', p_reason,
      'rescheduled_at', now()
    )
  where id = p_booking_id
