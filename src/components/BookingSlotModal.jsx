    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-6xl h-auto max-h-[94dvh] sm:max-h-[92vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
                <div className="shrink-0 px-4 sm:px-7 py-2.5 sm:py-3.5 border-b border-gray-100 bg-linear-to-r from-primary-light via-white to-secondary-light">
                    <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1.5">
                            <div className="inline-flex px-2.5 py-0.5 bg-white/90 text-secondary text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-full border border-secondary/20">
                                Step {step} of 2
                            </div>
                            <div>
                                <h2 className="text-lg sm:text-2xl font-display font-bold text-primary-dark leading-tight">
                                    {step === 1 ? 'Choose a Date' : 'Choose Time Slot'}
                                </h2>
                                <p className="hidden sm:block text-xs sm:text-sm text-gray-600 mt-0.5">
                                    {step === 1 ? (
                                        <>Pick an available date for <span className="font-semibold text-primary-dark">{selectedCourt.name}</span>.</>
                                    ) : (
                                        <>Now choose an available time slot for <span className="font-semibold text-primary-dark">{selectedCourt.name}</span>.</>
                                    )}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2.5 sm:mt-3.5">
                        ...
                        <div className="bg-secondary-light/60 rounded-lg sm:rounded-xl border border-secondary/30 px-2.5 sm:px-3 py-2 sm:py-2.5 shadow-sm">
                            <p className="text-[10px] sm:text-xs uppercase tracking-wider text-secondary font-bold flex items-center gap-1">
                                <CalendarDays size={12} /> Date Selected
                            </p>
                            <p className="text-xs sm:text-base font-bold text-primary-dark mt-0.5">{format(selectedDate, 'MMMM d, yyyy')}</p>
                        </div>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto px-4 sm:px-7 py-4 sm:py-5">
                    {step === 2 && (
                        <div className="mb-6 rounded-2xl border border-secondary/20 bg-secondary-light/40 px-4 py-3">
                            <p className="text-xs font-bold uppercase tracking-wider text-secondary">Selected Date</p>
                            <p className="mt-1 text-base font-semibold text-primary-dark">{format(selectedDate, 'MMMM d, yyyy')}</p>
                        </div>
                    )}
